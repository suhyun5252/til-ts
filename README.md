...
iu.push("소녀시대"); // js 에서 벼열이 바뀌므로 오류 없음. [30,"아이유", "소녀시대"]
...
let actors2: [name: string, gae: number] = ["이병헌", 50];
...
let sampleAges: [number, number] = ages; //가능
// let sampleAges2 : [string,number] = ages; // 타입이 맞지 않아서 불가능
// let sampleAges3 : [number,number,number] = ages; //요소 개수가 맞지 않아서 불가능
...
// 변수 즉 객체리터럴이 아닌 경우는 초가 검사를 ts가 안한다.
const bpink1: TAge = bpink; //이게 된다!(초과 검사 안함)
...
// 타입스크립트는 가능하면 타입 유추에 의해서 오류가 발생하는 것을 제거 가능하면 해주자.
interface Dog {
  name: string;
  age: number;
}
